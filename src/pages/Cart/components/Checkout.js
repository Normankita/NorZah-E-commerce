...
export const Checkout = ({setCheck}
    ...
    useEffect(()=>{
        async function attrib(){
          const userd = await fetchUser()
          console.log(userd)
          setUser(userd)
        }
        attrib()
    ...
    try{
       const data = await sendOrders(useData)
        clearCart()
        
       data? navigate("/order-summary", { state:{status: true, data:data} }) : navigate("/order-summary", {state:{status:false}})
    }catch(error){
        navigate("/order-summary", {state:{status:false, error:error}})
    }
